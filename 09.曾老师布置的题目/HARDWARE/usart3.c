...
extern u32 pwmzkb2;
extern u8 temp,humi;
extern u8 temp_max,temp_min;
extern u8 distant_max,distant_min;//单位cm
extern u32 arr,psc;
/*
 * 函数名：USART3_Config
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收中断 
		{
			...
			else if(rec_data=='E')		                         //如果E，表示是命令信息传送的结束位
			{
				...
				else if(strcmp("temp_max++",(char *)Uart3_Rx)==0)    temp_max++;
				else if(strcmp("temp_max--",(char *)Uart3_Rx)==0)    temp_max--;
				else if(strcmp("temp_min++",(char *)Uart3_Rx)==0)    temp_min++;
				else if(strcmp("temp_min--",(char *)Uart3_Rx)==0)    temp_min--;
				else if(strcmp("distant_max++",(char *)Uart3_Rx)==0)    distant_max++;
				else if(strcmp("distant_max--",(char *)Uart3_Rx)==0)    distant_max--;
				else if(strcmp("distant_min++",(char *)Uart3_Rx)==0)    distant_min++;
				else if(strcmp("distant_min--",(char *)Uart3_Rx)==0)    distant_min--;			
				else if(strcmp("arr++",(char *)Uart3_Rx)==0)    {arr=arr+1000;	TIM2_PWM_Init(arr,psc); }
				else if(strcmp("arr--",(char *)Uart3_Rx)==0)    {arr=arr-1000;	TIM2_PWM_Init(arr,psc); }
				else if(strcmp("psc++",(char *)Uart3_Rx)==0)    {psc=psc+10;	TIM2_PWM_Init(arr,psc); }
				else if(strcmp("psc--",(char *)Uart3_Rx)==0)    {psc=psc-10;	TIM2_PWM_Init(arr,psc); }
