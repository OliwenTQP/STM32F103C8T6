...
void TIM2_PWM_Init(u16 arr, u16 psc)
{
	...
	GPIO_InitStructure.GPIO_Pin= GPIO_Pin_0;//引脚1GPIO_Pin_1
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure); //初始化引脚GPIOA0
	...
	TIM_OCInitTypeStrue.TIM_OCPolarity=TIM_OCPolarity_High; //输出有效电平为低电平
	TIM_OCInitTypeStrue.TIM_OutputState=TIM_OutputState_Enable; //使能PWM输出
	TIM_OCInitTypeStrue.TIM_Pulse = 0; //设置待装入捕获比较寄存器的脉冲值
	TIM_OC2Init(TIM2, &TIM_OCInitTypeStrue); //根TIM_OCInitTypeStrue参数初始化定时器2通道1
